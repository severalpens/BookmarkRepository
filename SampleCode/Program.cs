...
#region snippet1
builder.Services.AddDbContextFactory<BookmarkContext>(opt =>
    opt.UseSqlite($"Data Source={nameof(BookmarkContext.BookmarksDb)}.db"));
#endregion
...
// filters
builder.Services.AddScoped<IBookmarkFilters, GridControls>();

// query adapter (applies filter to bookmark request).
...
var options = scope.ServiceProvider.GetRequiredService<DbContextOptions<BookmarkContext>>();
